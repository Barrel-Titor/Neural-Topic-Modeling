...
Original file is located at
    https://colab.research.google.com/drive/19_C_94Ry1RkND2GIvgHGTLbCp-P46WjG
"""
...
class Data(object):
    ...
    def __init__(self, batch_size=1, data_path='', device="cuda:0"):
        '''
        ...
        device: mxnet device context, default mx.cpu(0)
        ...
        for k, v in self.data.items():
            if v is not None and v.shape[0] < self.batch_size:
                print(
                    'NOTE: Number of samples for {0} is smaller than batch_size ({1}<{2}). Duplicating samples to exceed batch_size.'.format(
                ...
                if type(v) is np.ndarray:
                    ...
                    self.data[k] = torch.tile(v, (self.batch_size // v.shape[0] + 1, 1))
    ...
    def dataloader(self, data, batch_size, shuffle=True):
        ...
        if data is None:
            ...
            if type(data) is np.ndarray:
                return DataLoader(data, batch_size, drop_last=True, shuffle=shuffle)
            else:
                dataset_train = TensorDataset(data)
                return DataLoader(dataset_train, batch_size, shuffle=False, drop_last=True)
        ...
        if self.dataiters[key] is None:
            ...
            if type(self.data[key]) is np.ndarray:
                try:
                    ...
                    if pad_width > 0:
                        mb = torch.cat((mb, torch.zeros((self.batch_size, pad_width))))
                        mb = torch.cat((mb, torch.zeros((self.batch_size, pad_width))))
                    return mb.to(self.device)
                except:
                ...
                try:
                    mb = self.dataiters[key].__next__().data[0].to(self.device)
                    return mb
                except:
                    self.dataiters[key].hard_reset()
                    mb = self.dataiters[key].__next__().data[0].to(self.device)
    ...
    def visualize_series(y, ylabel, file, args, iteration, total_samples, labels=None):
        ...
        if len(y) > 0:
            ...
            newlabel = np.around(np.linspace(0, iteration, num=len(ax.get_xticks()) - 2)).astype(
                'int')  # labels of the xticklabels: the position in the new x-axis
    ...
    def load(self, path=''):
        ...
        '''
        data = [np.empty((1, 1)) for data in
                ['train', 'valid', 'test', 'train_with_labels', 'valid_with_labels', 'test_with_labels']]
...
class ENet(torch.nn.Module):
    ...
    def init_weights(self, weights=None):
        ...
        if self.weights_file != '' and weights is None:
            try:
                self.load_state_dict(self.weights_file).to(self.device)
        ...
        if weights is not None:
            ...
            for p, w in zip(self.parameters(), weights):
                if w is not None:
                    ...
                    torch.nn.init.constant(p.data, torch.tensor(w.squeeze())).to(self.device)
            #                    if self.freeze:
            #                       p.lr_mult = 0.
            print('NOTE: Loaded encoder weights from ' + source + '.')
            if self.freeze:
            ...
            loaded = True
        if not loaded:
            self.apply(self.initialize).to(self.device)
...
class Compute(object):
    ...
    def __init__(self, data, Enc, Dec, Dis_y, args):
        ...
        weights_enc = Enc.parameters()
        weights_dec = Dec.parameters()
        weights_dis_y = Dis_y.parameters()
        ...
        if self.args['optim'] == 'Adam':
            ...
            optimizer_enc = torch.optim.Adam(weights_enc, lr=self.args['learning_rate'], betas=(0.99, 0.99))
            optimizer_dec = torch.optim.Adam(weights_dec, lr=self.args['learning_rate'], betas=(0.99, 0.99))
            optimizer_dis_y = torch.optim.Adam(weights_dis_y, lr=self.args['learning_rate'], betas=(0.99, 0.99))
        ...
        if self.args['optim'] == 'Adadelta':
            ...
            optimizer_enc = torch.optim.Adadelta(weights_enc)
            optimizer_dec = torch.optim.Adadelta(weights_dec)
            optimizer_dis_y = torch.optim.Adadelta(weights_dis_y)
        if self.args['optim'] == 'RMSprop':
            ...
            optimizer_enc = torch.optim.RMSprop(weights_enc, lr=self.args['learning_rate'], eps=1e-10, alpha=0.9)
            optimizer_dec = torch.optim.RMSprop(weights_dec, lr=self.args['learning_rate'], eps=1e-10, alpha=0.9)
            optimizer_dis_y = torch.optim.RMSprop(weights_dis_y, lr=self.args['learning_rate'], eps=1e-10, alpha=0.9)
        if self.args['optim'] == 'SGD':
            args_dict = {'learning_rate': self.args['learning_rate'], 'wd': self.args['weight_decay'],
                         'rescale_grad': 1., 'momentum': 0.0, 'lazy_update': False}
            ...
            optimizer_enc = torch.optim.SGD(weights_enc, lr=self.args['learning_rate'],
                                            weight_decay=self.args['weight_decay'], momentum=0.0)
            optimizer_dec = torch.optim.SGD(weights_dec, lr=self.args['learning_rate'],
                                            weight_decay=self.args['weight_decay'], momentum=0.0)
            optimizer_dis_y = torch.optim.SGD(weights_dis_y, lr=self.args['learning_rate'],
                                              weight_decay=self.args['weight_decay'], momentum=0.0)
