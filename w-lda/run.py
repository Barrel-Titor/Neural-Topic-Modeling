...
Original file is located at
    https://colab.research.google.com/drive/1ShttCdSuSzSUDFsoVxGZBqIwW-xIg4Af
"""
...
def run_experiment(Compute, Domain, Encoder, Decoder, Discriminator_y, args):
    ...
    if args['use_kd']:
        ...
        data = Domain(batch_size=args['batch_size'], data_path=args['data_path'], device=device, saveto=args['saveto'])
    ...
    if type(data.data['train']) is np.ndarray:
        ...
        mean_length = torch.mean(torch.sum(data.data['train'][:, :vocab_size], axis=1))[0]
    ...
    Enc = Encoder(device=device, batch_size=args['batch_size'], input_dim=args['ndim_x'], ndim_y=args['ndim_y'],
    ...
    Dec = Decoder(device=device, batch_size=args['batch_size'], output_dim=args['ndim_x'], ndim_y=args['ndim_y'],
    ...
    Dis_y = Discriminator_y(device=device, batch_size=args['batch_size'], ndim_y=args['ndim_y'],
    ...
    if args['enc_weights']:
        Enc.load_state_dict(args['enc_weights'], device=device)
    else:
        Enc.init_weights()
    if args['dec_weights']:
        Dec.load_state_dict(args['dec_weights'], device=device)
    else:
        Dec.init_weights()
    Dis_y.init_weights()
        ...
        for i in epochs:
            ...
            if i == (args['max_iter'] - 1) or (args['eval_stats_every'] > 0 and i % args['eval_stats_every'] == 0):
                ...
                if args['domain'] == 'synthetic':
                    ...
                    topic_words, params = get_topic_words_decoder_weights(Dec, data, device, decoder_weights=False)
                    ...
                    topic_words, params = get_topic_words_decoder_weights(Dec, data, device, decoder_weights=True)
                    topic_uniqs = calc_topic_uniqueness(topic_words)
