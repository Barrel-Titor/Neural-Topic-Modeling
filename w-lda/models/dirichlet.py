...
Original file is located at
    https://colab.research.google.com/drive/1bvY-Ei4LgT-0wuqHWgwltwg8a7nCWImP
"""
...
class Encoder(ENet):
    ...
    '''
    def __init__(self, device, batch_size, input_dim, n_hidden=64, ndim_y=16, ndim_z=10, n_layers=0, nonlin=None,
        ...
        Args
        ----
        device: mxnet device context, No default
...
#                self.main.add_module('Linear',nn.Linear(in_units=in_units,))
...
#            self.main.add(nn.Dense(ndim_y, in_units=in_units, activation=None))
        ...
        for i in range(n_layers):
            self.main.add_module('Linear',nn.Linear(in_features=in_units,out_features=n_hidden[i]))
            if nonlin:
                if nonlin=='sigmoid':
                    self.main.add_module(nonlin, nn.Sigmoid())
                elif nonlin=='tanh':
                    self.main.add_module(nonlin, nn.Tanh())
                elif nonlin=='Relu':
                    self.main.add_module(nonlin, nn.ReLU())
                elif nonlin=='Leaky ReLU':
                    self.main.add_module(nonlin,nn.LeakyReLU())
            in_units = n_hidden[i]
        self.main.add_module('Out', nn.Linear(in_features=in_units,out_features=ndim_y))
...
class Decoder(DNet):
    ...
    '''
    def __init__(self, device, batch_size, output_dim, ndim_y=16,  n_hidden=64, n_layers=0, nonlin='',
...
#            self.main.add(nn.Dense(n_hidden[0], in_units=ndim_y, activation=None))
        ...
        self.main=nn.Sequential()
        self.main.add_module('Out', nn.Linear(in_features=ndim_y,out_features=n_hidden[0]))
    ...
    def y_as_topics(self, eps=1e-10):
      ...
      #  return mx.nd.array(y)
        return torch.tensor(y, dtype=float)
...
class Discriminator_y(ENet):
    ...
    def __init__(self, device, batch_size, output_dim=2, ndim_y=16, n_hidden=64, n_layers=0, nonlin='sigmoid',
        ...
        for i in range(n_layers):
            ...
            if nonlin:
                if nonlin=='sigmoid':
                    ...
                    self.main.add_module(nonlin,nn.LeakyReLU())
            in_units = n_hidden[i]
        self.main.add_module('Out', nn.Linear(in_features=in_units,out_features=output_dim))
